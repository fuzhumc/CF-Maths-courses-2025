...
(非常不严格的)\textbf{定义}
对于函数\(f:[a,b] \to \mathbb{R}\),如果\({} f(x)\geq0 {}\)我们称\(f\)的图像与\(x\)轴所围成的面积为\(f\)从\(a\)到\(b\)的积分,记为\(\int_a^b f(x)\mathrm{d}x\).如果\(f\)是变号的,则记\(\int_a^b f(x)\mathrm{d}x=\int_a^b f^+(x)\mathrm{d}x-\int_a^b f^-(x)\mathrm{d}x\).
...
\item
  ...
  \item
    对任意区间\([a,b]\),\(\int_a^b f(x) \mathrm{d}x \geq 0\).
  \item
    考虑一串区间\(a_0 \leq a_1 \leq a_2 \leq \cdots \leq a_n\),有\(\sum\limits_{j=0}^{n-1} \int_{a_j}^{a_{j+1}} f(x)\mathrm{d}x=\int_{a_0}^{a_n}f(x)\mathrm{d}x\).
  \item
    令\(n \to \infty\),是否仍然成立?
  \item
    这是在说什么事情?
