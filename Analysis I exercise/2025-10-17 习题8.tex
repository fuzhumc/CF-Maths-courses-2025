...
\item
  ...
  f(x)=
  \begin{cases}
  e^{\frac{1}{\left\lvert x \right\rvert^2-1}}, & \left\lvert x \right\rvert \leq 1 \\
...
\item
  ...
  \[
  \left\lvert f(x)-f(y) \right\rvert \leq k \left\lvert x-y \right\rvert ^\alpha,\forall x,y \in [a,b]
  \]
  ...
  \end{enumerate}
\item
  设\(f(x):\mathbb{R} \to \mathbb{R}\)为Lipschitz函数,我们称\(E=\{(x,f(x)) \in \mathbb{R}^2 \mid x \in \mathbb{R}\}\)为\textbf{Lipchitz图像}.设\(f(x)\)在\(x=x_1\)处可导,向量\((1,f’(x_1))\)称为图像在\(x=x_1\)处的\textbf{切向量},\(\nu(x_1)=\frac{1}{\sqrt{1+ \left\lvert f’(x_1) \right\rvert ^2}}\left( f’(x_1),-1 \right)\)称为此处的\textbf{单位外法向量}.现考虑两点\(X_1=\left( x_1,f\left( x_1 \right) \right),X_2=\left( x_2,y_2 \right)\),其中\(y_2 \geq f\left( x_2 \right)\).定义\(dist \left( X,E \right)=\inf_{Y \in E} \left\lvert X-Y \right\rvert\).若存在\(C>0\)使得
  \[
  \frac{1}{C}dist \left( X_2,E \right) \leq \left\lvert X_1-X_2 \right\rvert \leq Cdist \left( X_2,E \right)
  \]
