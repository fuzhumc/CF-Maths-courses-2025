...
\item
  ...
  证明:\(f\)在任意点处都连续.
\item
  设\(A \subseteq \mathbb{R}\)非空,定义\({} D(A)=\sup\limits_{x,y \in A} \vert x-y \rvert {}\).设\(f\)为有界函数,定义\(\omega(f,x)=\inf\limits_{U\text{为}x\text{的开邻域}}D(f(U))\).证明:
  ...
  \item
    对任意\(\epsilon > 0,E=\{ x \in \mathbb{R} \mid \omega(f,x) \geq \varepsilon \}\)为闭集.
  \end{enumerate}
\item
  ...
  \tightlist
  \item
    对任意\(\varepsilon > 0,E_\varepsilon=\{ x \in [a,b] \mid \vert \lim\limits_{y \to x} f(y)-f(x) \rvert > \varepsilon \}\)为有限集.
  \item
    \(f\)的间断点集是可数集.
  \end{enumerate}
\end{enumerate}
