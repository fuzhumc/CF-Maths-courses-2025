...
\begin{prop}[局部性]
    设$f:I\to\mathbb{R},J \subseteq I$,$a$为$J$的内点或端点.若$\lim\limits_{x\to a}f(x)=l$,则$\lim\limits_{x\to a}f(x)\vert_J=l$
\end{prop}
...
\begin{deff}
    称$f$在$a$处\textbf{连续},若$\lim\limits_{x \to a} f(x) = f(a)$.
\end{deff}
...
\begin{deff}[左右连续]
    称$f$在$a$处\textbf{左连续},若$\lim\limits_{x \to a^-} f(x) = f(a)$.

    称$f$在$a$处\textbf{右连续},若$\lim\limits_{x \to a^+} f(x) = f(a)$.
\end{deff}

\begin{deff}
    设$I$为区间,$a$为$I$内点,$f:I\setminus\{a\}\to\mathbb{R}$.
    称$\tilde{f}:I\to\mathbb{R}$为$f$的\textbf{连续延拓},若$\tilde{f}$满足
...
\begin{deff}[Lipschitz函数]
    设$f:I\to\mathbb{R},k>0$.称$f$为$k$-\textbf{Lipschitz函数},若对于任意$x,y\in I$,
    都有$\left\lvert f(x)-f(y) \right\rvert \leqslant k \left\lvert x-y \right\rvert$.
\end{deff}

\begin{deff}[一致连续]
    设$I$为区间,$f:I\to \mathbb{R}$.称$f$在$I$上\textbf{一致连续},
