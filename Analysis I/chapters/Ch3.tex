...
\begin{deff}
    ...
    称$(u_n)$等价于$(v_n)$,若$\lim\limits_{n\to\infty}\frac{u_n}{v_n}=1$.
...
\begin{deff}
    设序列$u=(u_n),v=(v_n)$从某项开始非$0$.
    称$u$受$v$控制,若存在$N \in \mathbb{N}$,当$n>N$时,$\left\lvert \frac{u_n}{v_n} \right\rvert$有界.
    记为$u = O(v)$或$u_n = O(v_n)$.
\end{deff}

\begin{deff}
    设序列$u=(u_n),v=(v_n)$从某项开始非$0$.
    称$u$在$v$前可忽略,若$\lim\limits_{n\to\infty}\frac{u_n}{v_n}=0$.
    记为$u = o(v)$或$u_n = o(v_n)$.
\end{deff}
