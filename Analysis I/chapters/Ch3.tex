...
\begin{proof}
    ...
    令$\varepsilon=\frac{\left\lvert l-\tilde{l} \right\rvert}{47}$,则

    存在$N_1$,当$n>N_1$时,$\left\lvert u_n - l \right\rvert < \varepsilon$.

    存在$N_2$,当$n>N_2$时,$\left\lvert u_n - \tilde{l} \right\rvert < \varepsilon$.

    令$N=\max\{N_1,N_2\}$,则当$n>N$时,有$$\left\lvert l - \tilde{l} \right\rvert \leq \left\lvert l - u_n \right\rvert + \left\lvert u_n - \tilde{l} \right\rvert < 2\varepsilon = \frac{2\left\lvert l-\tilde{l} \right\rvert}{47}$$
...
\begin{deff}
    称序列$(u_n)$发散至$+\infty(-\infty)$,若对于任意$M \in \mathbb{R}$,存在$N \in \mathbb{N}$,当$n>N$时,$u_n>M(u_n<M)$.
    记为$\lim\limits_{n\to\infty}u_n=+\infty(-\infty)$
\end{deff}

\begin{rqq}
    此时不能说$(u_n)$收敛,也应当称$(u_n)$极限不存在.
\end{rqq}
...
\begin{deff}
    称两序列$(u_n),(v_n)$是\textbf{相邻}的,若$(u_n)$单调增,$(v_n)$单调减,
...
\begin{deff}
    设$A \subset \mathbb{R}$,称$A$在$\mathbb{R}$中\textbf{稠密},若对于任意$a,b \in \mathbb{R},a<b$,
    存在$x \in A$,使得$a<x<b$.
\end{deff}
...
\section{序列的比较关系}\label{sec:sequences_comparison}

\begin{deff}
    设序列$u=(u_n),v=(v_n)$从某项开始非$0$.
    称$(u_n)$\textbf{等价}于$(v_n)$,若$\lim\limits_{n\to\infty}\dfrac{u_n}{v_n}=1$.
...
\begin{deff}
    设序列$u=(u_n),v=(v_n)$从某项开始非$0$.
    称$u$受$v$\textbf{控制},若存在$N \in \mathbb{N}$,当$n>N$时,$\left\lvert \dfrac{u_n}{v_n} \right\rvert$有界.
    记为$u = O(v)$或$u_n = O(v_n)$.
\end{deff}

\begin{deff}
    设序列$u=(u_n),v=(v_n)$从某项开始非$0$.
    称$u$在$v$前\textbf{可忽略},若$\lim\limits_{n\to\infty}\dfrac{u_n}{v_n}=0$.
    记为$u = o(v)$或$u_n = o(v_n)$.
\end{deff}
