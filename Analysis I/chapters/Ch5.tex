...
\begin{deff}
    称映射$f':\begin{array}[t]{ccc} I & \to & \mathbb{R} \\ x & \tp & f'(x) \end{array}$
    为$f$的\textbf{导函数}.
\end{deff}
...
\begin{deff}[左右导数]
    ...
    称$f$在$a$处\textbf{右可导},若$\lim\limits_{h\to0^+}\frac{f(a+h)-f(a)}{h}$存在.极限值记为$f'(a^+)$.

    称$f$在$a$处\textbf{左可导},若$\lim\limits_{h\to0^-}\frac{f(a+h)-f(a)}{h}$存在.极限值记为$f'(a^-)$.
\end{deff}

\begin{thm}[导数极限定理]
    设$f\in\mathcal{C}(I,\mathbb{R})$在$I\setminus\{a\}$上可导.
    若$\lim\limits_{x\to a}f'(x)$存在,则$\lim\limits_{x\to a}\frac{f(x)-f(a)}{x-a}$存在,
    且$\lim\limits_{x\to a}\frac{f(x)-f(a)}{x-a}=\lim\limits_{x\to a}f'(x)$.
\end{thm}
...
\begin{thm}[Fermat]
    设$f:I\to\mathbb{R}$,$a$为$I$内点.若$f$在$a$处可导且$f$在$a$处取到极值,则$f'(a)=0$.
\end{thm}
...
\begin{thm}[L'Hôpital]
    设$f,g:[a,b]\to\mathbb{R},c\in[a,b]$,$f,g$在$c$附近可导,且
    $\lim\limits_{x\to c}f(x)=\lim\limits_{x\to c}g(x)=0,g(x)\neq0,\lim\limits_{x\to c}\dfrac{f'(x)}{g'(x)}$存在,
    则$\lim\limits_{x\to c}\dfrac{f(x)}{g(x)}$存在且$\lim\limits_{x\to c}\dfrac{f(x)}{g(x)}=\lim\limits_{x\to c}\dfrac{f'(x)}{g'(x)}$
\end{thm}

\begin{thm}[L'Hôpital]
    设$f,g:[a,b]\to\mathbb{R},c\in[a,b]$,$f,g$在$c$附近可导,且
    $\lim\limits_{x\to c}g(x)=+\infty,\lim\limits_{x\to c}\dfrac{f'(x)}{g'(x)}$存在,
    则$\lim\limits_{x\to c}\dfrac{f(x)}{g(x)}$存在且$\lim\limits_{x\to c}\dfrac{f(x)}{g(x)}=\lim\limits_{x\to c}\dfrac{f'(x)}{g'(x)}$
\end{thm}
...
\begin{prop}
    设$f:I\to\mathbb{C}$可导,则$\bar{f}$也可导,且$(\bar{f})'=\bar{(f')}$.
\end{prop}
...
\begin{deff}
    设$f:I\to\mathbb{R}$,称集合$\varepsilon_f=\{(x,y)\in I\times\mathbb{R}\mid f(x)\leqslant y\}$
    为$f$的\textbf{上方集}.
\end{deff}
...
\begin{prop}
    设$f$凸,则$f(x)\geqslant f'(x_0)(x-x_0)+f(x_0)$.
\end{prop}
