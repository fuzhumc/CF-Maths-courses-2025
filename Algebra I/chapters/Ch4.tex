...
\begin{rqq}
    在不混淆的情况下,可记$0 = 0_A, 1 = 1_A$.
\end{rqq}

\begin{rqq}
    设$(A, +, *)$为环,$a \in A, n \in \bbN^*$.记$\underbrace{a + a + \cdots + a}_{n \text{个}} = na, \underbrace{a * a * \cdots * a}_{n \text{个}} = a^n$.

    特别地,约定$\forall a \in A^*, 0a = 0, a^0 = 1$.
\end{rqq}
...
\begin{deff}
    设$(A, +, *)$为环,$a \in A$.称$a$为$A$的一个\textbf{可逆元}或\textbf{单位},若$a$可逆.

    $A$中单位的全体记为$A^{\times}$.
\end{deff}

\begin{rqq}
    设$(A, +, *)$为环,则$(A^{\times}, *)$为群,称为$A$的\textbf{单位群}.
\end{rqq}
...
\begin{deff}
    设$(A, +_A, *_A), (B, +_B, *_B)$为环,$\varphi \in \calF(A, B)$.称$\varphi$为\textbf{环同态},若
    \begin{enumerate}
        \item $\varphi(1_A) = 1_B$.
        \item $\forall x, y \in A, \varphi(x +_A y) = \varphi(x) +_B \varphi(y)$.
        \item $\forall x, y \in A, \varphi(x *_A y) = \varphi(x) *_B \varphi(y)$.
    \end{enumerate}
\end{deff}

\begin{deff}
    设$\varphi: A \to B$为环同态,称$\varphi^{-1}(1_B) = \{ a \in A \mid \varphi(a) = 1_B \}$为$\varphi$的\textbf{核},记为$\ker \varphi$.
\end{deff}
...
\begin{prop}
    设$\varphi: A \to B$为环同态,则$\varphi$为环同构$\iff$存在环同态$\psi: B \to A, \psi \circ \varphi = \id_A, \varphi \circ \psi = \id_B$.
\end{prop}

\begin{prop}
    设$\varphi: A \to B$为环同态,则$\varphi$为单同态$\iff \ker \varphi = \{ e \} \iff \lvert \ker \varphi \rvert = 1$.
\end{prop}

\begin{rqq}
    与群同态不同,环同态中没有$\coker \varphi$.
\end{rqq}

\section{理想}

\begin{deff}\label{def:ideal}
...
\begin{rqq}
    若$A$不交换,则Définition \ref{def:ideal}中的$I$称为\textbf{左理想},类似地可以定义\textbf{右理想}.
\end{rqq}
...
\begin{thm}
    设$\varphi: A \to B$为环同态,则存在唯一环同构$\bar{\varphi}: A / \ker \varphi \to \im \varphi$使得$\varphi = i \circ \bar{\varphi} \circ \pi$.
    其中$i: \im \varphi \to B$为自然嵌入(单同态),$\pi: A \to B / \ker \varphi$为自然投影(满同态).
\end{thm}

\begin{prop}
    设$\varphi: A \to B$为满同态,则$A / \ker \varphi \cong B$.
\end{prop}
...
\begin{deff}
    设$(A, +, *)$为交换环,$P$为$A$的理想.称$P$为$A$的一个\textbf{素理想},若$P \neq A$且$\forall x, y \in A, x * y \in P \Rightarrow x \in P \text{且} y \in P$
\end{deff}
...
\begin{deff}
    设$(A, +, *)$为环,$E$为非空集合.称映射$*: \begin{array}[t]{ccc} A \times E & \to & E \\ (\lambda, x) & \tp & \lambda * x \end{array}$为$A$在$E$上的一个\textbf{外部合成法则}或\textbf{外部运算}.
\end{deff}
