...
\begin{rqq}
    设$(A, +, *)$为环,$a \in A, n \in \mathbb{N}^*$.记$\underbrace{a + a + \cdots + a}_{n \text{个}} = na, \underbrace{a * a * \cdots * a}_{n \text{个}} = a^n$.
...
\begin{deff}
    设$(A, +_A, *_A), (B, +_B, *_B)$为环,$\varphi \in \mathcal{F}(A, B)$.称$\varphi$为\textbf{环同态},若
    \begin{enumerate}
        \item $\varphi(1_A) = 1_B$.
...
\begin{rqq}
    与群同态不同,环同态中没有$\mathrm{coKer} \varphi$.
\end{rqq}
