...
\begin{deff}
    ...
    此时记$u:\begin{array}[t]{ccc} E & \to & F \\ x & \tp & y \end{array}$,也记$u(x)=y$.
...
\begin{prop}
    设$u: E \to F$,则$u \circ u^{-1} = \id_F, u^{-1} \circ u = \id_E$.
\end{prop}
...
\begin{lem}
    设$u: E \to F, v: F \to E$,若$u \circ v = \id_F, v \circ u = \id_E$,则$u, v$均为双射且互为逆映射.
\end{lem}

\begin{proof}
    由$u \circ v = \id_F$为双射知$u$为单射且$v$为满射,

    由$v \circ u = \id_E$为双射知$v$为单射且$u$为满射.
...
\begin{deff}
    设$u: E \to E$,称$u$为\textbf{对合},若$u \circ u = \id_E$.
\end{deff}
...
\begin{prop}
    设$E$为集合,则映射\[ \begin{array}[t]{ccc} \calP(E) & \to & \calF(E, \{ 0, 1 \}) \\ A & \tp & \bbI_A \end{array} \]为双射.
\end{prop}
...
\begin{deff}
    设$u: E \to F, A \subseteq E$,称$u \vert_A: \begin{array}[t]{ccc} A & \to & F \\ x & \tp & u(x) \end{array}$为$u$在$A$上的\textbf{限制}.
\end{deff}
...
\begin{rqq}
    存在从$E$到$E / \calR$的满射$\pi: \begin{array}[t]{ccc} E & \to & E / \calR \\ x & \tp & \bar{x} \end{array}$.
\end{rqq}
