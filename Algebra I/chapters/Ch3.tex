...
\begin{deff}
    ...
    \begin{enumerate}
        \item $*$结合.\label{def1}
        \item $G$中有中性元.\label{def2}
        \item $G$中的每个元素均可逆.\label{def3}
    \end{enumerate}
\end{deff}
...
\begin{rqq}
    称满足\ref{def1}的元群为\textbf{半群}.
    称满足\ref{def1}和\ref{def2}的元群为\textbf{幺半群}.
\end{rqq}

\begin{exer}
    设$(G, *)$为群,$A$为非空集合,定义$\mathcal{F}(A, G)$上的一个内部合成法则如下:$$\otimes: \begin{array}[t]{ccc} \mathcal{F}(A, G) \times \mathcal{F}(A, G) & \to & \mathcal{F}(A, G) \\ (f, g) & \mapsto & f \otimes g: \begin{array}[t]{ccc} A & \to & G \\ a & \mapsto & f(a) * g(a) \end{array} \end{array}$$
    
    证明:$(\mathcal{F}(A, G), \otimes)$为群.
\end{exer}
...
\begin{prop}
    设$\varphi: G \to H$为群同态,则$\varphi$为单射$\iff \mathrm{Ker} \varphi = \{ e \} \iff \lvert \mathrm{Ker} \varphi \rvert = 1$.
\end{prop}
...
\begin{prop}
    ...
    则$\mathrm{Ker} p_1 = \{ e_1 \} \times G_2, \mathrm{Ker} p_2 = G_1 \times \{ e_2 \}$均为$G_1 \times G_2$的正规子群.
\end{prop}
...
\begin{rqq}
    Proposition \ref{prop2}给出了$G$的一个划分,即给出了$G$上的一个等价关系.
\end{rqq}
...
\begin{thm}[典范分解定理]\label{thm1}
    设$\varphi: G \to H$为群同态,则存在唯一群同构$\bar{\varphi}: G / \mathrm{Ker} \varphi \to \mathrm{Im} \varphi$使得$\varphi = i \circ \bar{\varphi} \circ \pi$.
    其中$i: \mathrm{Im} \varphi \to H$为自然嵌入(单同态),$\pi: G \to G / \mathrm{Ker} \varphi$为自然投影(满同态).
\end{thm}

\begin{rqq}
    Théorème \ref{thm1}的意义为:将群同态的研究归结为对单同态(自然嵌入$i$)与满同态(自然投影$\varphi$)的研究.
\end{rqq}

\begin{cor}
    若$G$有限,则$\lvert G \rvert = \lvert \mathrm{Ker} \varphi \rvert \lvert \mathrm{Im} \varphi \rvert$.
\end{cor}
...
\begin{deff}
    设$\varphi: A \to B$为群同态,称$B / \mathrm{Im} \varphi$为$\varphi$的\textbf{余核},记为$\mathrm{coKer} \varphi$.
\end{deff}

\begin{prop}
    设$\varphi: A \to B$为群同态,则$\varphi$为满同态$\iff \lvert \mathrm{coKer} \varphi \rvert = 1$.
\end{prop}

\begin{prop}
    设群同态$\varphi: A \to B, \pi: B \to \mathrm{coKer} \varphi$满足$\varphi \circ i = 0$,则对于任意$\psi: B \to C$且$\psi \circ \varphi = 0$,存在唯一$\beta: \mathrm{coKer} \varphi \to C$,使得$\psi = \beta \circ \pi$.
\end{prop}