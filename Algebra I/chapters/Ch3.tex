...
\begin{deff}
    设$E$为集合.称映射$*: \begin{array}[t]{ccc} E \times E & \to & E \\ (x,y) & \tp & x*y \end{array}$为$E$上的一个\textbf{内部合成法则}或\textbf{(内部)运算}.
...
\begin{exer}
    设$(G, *)$为群,$A$为非空集合,定义$\calF(A, G)$上的一个内部合成法则如下:\[ \otimes: \begin{array}[t]{ccc} \calF(A, G) \times \calF(A, G) & \to & \calF(A, G) \\ (f, g) & \tp & f \otimes g: \begin{array}[t]{ccc} A & \to & G \\ a & \tp & f(a) * g(a) \end{array} \end{array} \]
...
\begin{prop}
    设$\varphi: G \to H$为群同态,则$\varphi$为群同构$\iff$存在群同态$\psi: H \to G, \psi \circ \varphi = \id_G, \varphi \circ \psi = \id_H$.
\end{prop}
...
\begin{prop}
    ...
    \[ *': \begin{array}[t]{ccc} G / H \times G / H & \to & G / H \\ (aH, bH) & \tp & (a * b)H \end{array} \]
...
\begin{prop}
    存在自然投影$\pi: \begin{array}[t]{ccc} G & \to & G / H \\ a & \tp & aH \end{array}$满足$\pi(a * b) = \pi(a) *' \pi(b)$
\end{prop}
