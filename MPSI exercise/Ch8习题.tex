...
\item
  ...
  \item
    设实序列\(u\)递增,且满足: \[
     \forall p \in \mathbb{N} , \left\lvert u_{2^p}-u_{2^{p+1}} \right\rvert \leq \dfrac{1}{2^p}
     \] 求证\({} u {}\)收敛.
  \end{enumerate}
\item
  设两正序列\((u_n),(v_n)\)满足: \[
...
\item
  考虑以\(u_n=\dfrac{1}{n(n+1)(n+2)}\)为通项的实序列.确定三个实数\(a,b,c\)使得\(\forall n \in \mathbb{N}^*,u_n=\dfrac{a}{n}+\dfrac{b}{n+1}+\dfrac{c}{n+2}\).考虑序列\(v_n=\sum\limits_{k=1}^n u_n\)的极限.
\item
...
\item
  ...
  \forall n,p \in \mathbb{N},\left\lvert u_n \right\rvert \leq \alpha_p+\dfrac{p}{n+1}
  \] 求证序列\((u_n)\)趋于\(0\).
\item
  ...
  \item
    ...
    \item
      证明存在一项\(n_0\),使得当\(n \geq n_0\)时,有: \[
       \left\lvert \dfrac{u_{n_0}+u_{n_0+1}+\cdots+u_n}{n} \right\rvert \leq \dfrac{\varepsilon}{2}
       \] 下面\(n_0\)表示这一项.
    \item
      证明存在一项\(n_1\),使得当\(n \geq n_1\)时,有: \[
       \left\lvert \dfrac{u_1+u_2+\cdots+u_{n_0-1}}{n} \right\rvert \leq \dfrac{\varepsilon}{2}
       \] 下面\(n_0\)表示这一项.
    \item
      然后推出所宣称的结果.
    \end{enumerate}
...
\item
  我们考虑两个序列\(u\)与\(v\)发散至\(+\infty\),且\(\lim\limits_{n \to \infty} (u_{n+1}-u_n)=0\).给定\(\varepsilon>0\),考虑项\(n_0\),从这一项起有\(\left\lvert u_{n+1}-u_n \right\rvert \leq \varepsilon\).

  \begin{enumerate}
  \def\labelenumii{\arabic{enumii}.}
  \tightlist
  \item
    证明:对于任意实数\(x\)满足\(x \geq u_{n_0}\),存在序列中一项\(u_p\)满足\(\left\lvert u_p-x \right\rvert \leq \varepsilon\).
  \item
    使用\((v_n)\)发散至\(+\infty\)的事实,证明:对于任意实数\(x\),存在\(p,m\in\mathbb{N}\)满足\(\left\lvert (u_p-v_m)-x \right\rvert \leq \varepsilon\).
  \item
