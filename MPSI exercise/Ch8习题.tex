...
\tightlist
\item
  证明集合\(A=\{ \frac{k}{2^n} \mid k \in [0,2^n] \}\)在\([0,1]\)上稠密.
\item
  两个有下界实序列的乘积是否还是有下界序列?
\item
  设序列\((u_n)\)单调.证明:序列\(v_n=\frac{1}{n}\sum\limits_{k=1}^nu_k=\frac{1}{n}(u_1+u_2+\cdots+u_n)\)是单调的,且与序列\((u_n)\)有相同的单调性.
\item
  证明若某序列从某项开始递增,则其有下界.
\item
...
\item
  ...
  \item
    \(u_n=\frac{sin(n^2)}{n}\).
  \item
    \(u_n=\frac{a^n-b^n}{a^n+b^n},a,b>0\).
  \item
    \(u_n=\frac{n^3+5n}{5n^3+\cos n+\frac{1}{n^2}}\).
  \item
...
\item
  ...
  \item
    设实序列\(u\)递增,且满足: \[
     \forall p \in \mathbb{N} , \vert u_{2^p}-u_{2^{p+1}} \rvert \leq \frac{1}{2^p}
     \] 求证\({} u {}\)收敛.
  \end{enumerate}
\item
  设两正序列\((u_n),(v_n)\)满足: \[
  \forall n \in \mathbb{N},u_{n+1}=\sqrt{u_nv_n},v_{n+1}=\frac{u_n+v_n}{2}
  \] 证明这两个序列收敛于同一个极限.
\item
  设正序列\((u_n)\)满足:对于任意\(k \in \mathbb{N}\),集合\(\{ n \in \mathbb{N} \mid u_n > 10^{-k} \}\)有限.求证该序列收敛于\(0\).
\item
  考虑以\(u_n=\frac{1}{n(n+1)(n+2)}\)为通项的实序列.确定三个实数\(a,b,c\)使得\(\forall n \in \mathbb{N}^*,u_n=\frac{a}{n}+\frac{b}{n+1}+\frac{c}{n+2}\).考虑序列\(v_n=\sum\limits_{k=1}^n v_n\)的极限.
\item
  研究定义在\(\mathbb{N}\)上的序列\(u_n=\begin{cases} 1+\frac{1}{p}, & n=2p \\ 1-\frac{1}{p^2}, & n=2p+1 \end{cases}\)
\item
...
\item
  考虑如下定义的序列\((u_n)\)与\((v_n)\) \[
  u_n=\sum\limits_{k=0}^n \frac{1}{k!},v_n=u_n+\frac{1}{n \cdot n!}
  \]
  ...
  \item
    对于\(a,b \in \mathbb{R}\),用\(l\)的函数确定以\(\sum\limits_{k=0}^n \frac{ak+b}{k!}\)为通项的序列的极限.
  \end{enumerate}
\item
  证明序列\(u_n=\frac{5n^2+\sin n}{3(n+2)^2\cos \frac{n\pi}{5}}\)不收敛.
\item
  研究序列\(u_n=\left( 5 \sin \frac{1}{n^2} + \frac{1}{5} \cos n \right)^n\)的敛散性.
\item
  设实序列\((u_n)\)有界且使得序列\((u_{n+1}-u_n)\)单调.证明\((u_n)\)收敛.
\item
  设序列\(u\)满足:存在一个趋于\(0\)的序列\((\alpha_p)\)使得: \[
  \forall n,p \in \mathbb{N},\vert u_n \rvert \leq \alpha_p+\frac{p}{n+1}
  \] 求证序列\((u_n)\)趋于\(0\).
\item
  ...
  \item
    ...
    \item
      证明存在一项\(n_0\),使得当\(n \geq n_0\)时,有: \[
       \vert \frac{u_{n_0}+u_{n_0+1}+\cdots+u_n}{n} \rvert \leq \frac{\varepsilon}{2}
       \] 下面\(n_0\)表示这一项.
    \item
      证明存在一项\(n_1\),使得当\(n \geq n_1\)时,有: \[
       \vert \frac{u_1+u_2+\cdots+u_{n_0-1}}{n} \rvert \leq \frac{\varepsilon}{2}
       \] 下面\(n_0\)表示这一项.
    \item
      然后推出所宣称的结果.
    \end{enumerate}
...
\item
  ...
  v_n=\frac{u_1+2u_2+\cdots+nu_n}{n^2}
...
\item
  ...
  \item
    \({} w_n=\sqrt{1-\frac{1}{\ln n+1}}-1 {}\).
  \end{enumerate}
\item
  设\(u_0\geq0\),\(u_{n+1}=\frac{u_n}{1+u_n}\).通过考虑\(\frac{1}{u_n}\),证明\(u_n \sim \frac{1}{n}\).
\item
  \begin{enumerate}
  \def\labelenumii{\arabic{enumii}.}
  \tightlist
  \item
    证明: \[
     \forall k \in \mathbb{N}^*,\frac{1}{2\sqrt{k+1}} \leq \sqrt{k+1}-\sqrt{k} \leq \frac{1}{2\sqrt{k}}
     \]
  \item
    推出\(u_n=\sum\limits_{k=1}^n \frac{1}{\sqrt{k}}\)的简单等价.
  \end{enumerate}
\item
  使用符号\(o\)比较如下定义的四个序列: \[
